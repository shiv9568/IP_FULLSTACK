...
export const createEvent = async (req, res) => {
    try {
        const { eventPoster, eventName, eventSpeaker, dateTime, contactDetails, venue, description } = req.body;
        const uniId = req.uniId;
        const clubId = req.clubId;
        console.log("clubId is here ",clubId);

        if (!eventPoster || !eventName || !dateTime || !contactDetails || !venue || !description || !clubId) {
            return res.status(400).json({
                status: false,
        ...
        const existingEvent = await Event.findOne({ eventName, date:dateTime, uniId, club: clubId });
